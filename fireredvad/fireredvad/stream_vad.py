...
class FireRedStreamVad:
    ...
    def detect_full1(self, audio:np.ndarray):
        self.reset()
        feats, dur = self.audio_feat.extract(audio)
        if self.config.use_gpu:
            feats = feats.cuda()
        ...
        for t, raw_prob in enumerate(raw_probs):
            stream_vad_frame_result = self.postprocessor.process_one_frame(raw_prob)
            if stream_vad_frame_result.is_speech_start:
                result=1
            elif stream_vad_frame_result.is_speech_end:
                result=2
            #frame_results.append(stream_vad_frame_result)
        self.reset()
        return result
    ...
    @classmethod
    def results_to_timestamps(self, results):
        ...
        return timestamps
    def results_to_timestamps1(self, results):
        results = sorted(results, key=lambda r: r.frame_idx)
        # Get frame index (0-based)
        frame_timestamps = []
        start, end = -1, -1
